...
- 390px幅では画像一覧の最上位 `すべて / 日常 / キャラクター` が横スクロールに隠れたため、collection filterだけ折り返して全表示するよう修正した。その後、iPhoneミラーリング上の実機Safariで画像一覧、アルバム一覧、個別アルバム、ライトボックス、詳細遷移を確認し、横崩れ・画像切れ・主要操作の欠落がないことを確認した。
