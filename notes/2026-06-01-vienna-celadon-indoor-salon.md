# 2026-06-01 Vienna Celadon Indoor Salon

- Date seed: 2026-06-01, Asia/Tokyo
- Generated on: 2026-06-13, Asia/Tokyo
- City: Vienna
- Theme: indoor salon, office polish, reception, concert foyer, and apartment music night
- Common mood: polished interiors, brass, parquet, filtered light, quiet conversation
- Lucky color: celadon pistachio
- Lucky color age tone: glossy fresh pistachio for `18-19-adult` was intentionally unused; polished celadon for `20-24`; smoky pistachio for `25-29`
- Climate fit: mild early-June Vienna, mostly indoor, light air conditioning, seated/standing reception movement, no rain or waterfront dependence
- Selected categories: `office`, `formal`, `theater`, `home`
- Age bands: `25-29`, `20-24`, `25-29`, `20-24`
- Persona direction: selected trait mixes anchored in fashion interest, polish, and indoor social presence
- Exposure plan: tailored bare arms, wrap blouse, square-neck evening dress, home knit with tank and leggings
- Pose families: `close-crop-upper`, `using-object`, `adjusting-jacket`, `floor-sit`
- Movement readability notes: all actions are indoor and legible: portfolio check, reception cup turn, jacket adjustment, and record selection
- Style presets: `runway-board-illustration`, `anime-editorial`, `watercolor-couture`, `doll-like-3d`
- Saved path: `assets/daily/2026-06-01-vienna-celadon-indoor-salon/`
- Prompt version: `v2-short-generation`

## Results

- `01-office-looshaus-waistcoat-runway-note.webp`: accepted with style note. Looshaus-style office lobby, long waistcoat, cropped trousers, slingback pumps, portfolio, and belt adjustment create a quiet indoor office look. The output is more photo-adjacent than the intended runway-board illustration.
- `02-formal-palais-ferstel-wrap-anime.webp`: accepted. Palais Ferstel-style arcade reception, wrap blouse, pleated skirt over trousers, glass-bead pouch, and coffee-cup gesture create a formal but sociable indoor look.
- `03-theater-musikverein-satin-watercolor.webp`: accepted. Musikverein-style golden foyer, square-neck satin dress, cropped organza jacket, sheer socks, Mary Jane pumps, and program gesture create a theater look without repeating lounge seating.
- `04-home-josefstadt-record-doll-3d-note.webp`: accepted with style note. Josefstadt Altbau apartment salon, floor-sit record selection, satin wrap shorts over leggings, ballet flats, and hair ribbon create an indoor home look. The output is photo-adjacent rather than clearly doll-like 3D.

## Quality Notes

- All four files are separate images, not a collage.
- The set deliberately reduces current-month repetition by choosing a non-Japanese city, mostly indoor rooms, no rain-after-street emphasis, and no waterfront.
- Categories avoid the recent `market` / `gallery` / `lounge` / `transit` pattern.
- Places and time vary: Monday morning office lobby, late-afternoon reception arcade, evening concert foyer, and night apartment salon.
- Fashion focal points are specific: long waistcoat and slingbacks; wrap blouse and skirt-over-trousers; square-neck satin column dress and Mary Janes; satin wrap shorts over leggings and ballet flats.
- No readable brand text, logos, exact commercial product copy, celebrity likeness, school-uniform cues, or source-photo copying were requested.
